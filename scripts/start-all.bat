...
REM 2) Start Vite dev in a new window (first, so the app is available immediately)
pushd "%ROOT%"
...
echo.
echo [done] Frontend opened. Docling starts in a separate window; if it fails, check scripts\logs and run again:
