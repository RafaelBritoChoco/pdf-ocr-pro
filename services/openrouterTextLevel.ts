...
import { getBodyContentTaskDocling, getTablePreservationGuard } from '../prompts/doclingTemplates';

export interface ORTextLevelArgs {
...
export async function processTextLevelChunkOpenRouter(args: ORTextLevelArgs): Promise<string> {
  args.onApiCall();
  const task = getBodyContentTaskDocling() + '\n' + getTablePreservationGuard();
  ...
  try {
    ...
    console.warn('[OpenRouter][TextLevel] erro', e);
